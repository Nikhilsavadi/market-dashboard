...
def get_sector_heatmap():
    ...
    for theme, tickers in theme_tickers.items():
        ...
        if rs_scores:
            # Use actual signal RS scores — most accurate
            avg_rs = sum(rs_scores) / len(rs_scores)
            # Normalise RS rank (0-100) to a percentage-like scale for display
            display_val = round((avg_rs - 50) / 5, 2)  # centre at 0, ±10 range
            source = "signals"
        else:
            # Fallback: use parent sector 1m RS vs SPY
            sector_name = get_sector(tickers[0]) if tickers else None
            fallback_rs = sector_1m.get(sector_name, 0) if sector_name else 0
            display_val = round(fallback_rs, 2)
    ...
    # Build GICS sector rows from sector_rs cache
    sector_rows = []
    for name, d in sector_rs.items():
        rs1m = d.get("rs_vs_spy_1m")
        if rs1m is None:
    ...
    for theme, tickers in theme_tickers.items():
        ...
        meta = THEME_META.get(theme, {})
        theme_rows.append({
            "name": meta.get("label", theme).split(" ", 1)[-1],  # strip emoji
            "label": meta.get("label", theme),
            "type": "theme",
    ...
    # Sort themes by avg RS descending
    theme_rows.sort(key=lambda x: x["avg_rs"], reverse=True)
